...
for(const [engineName,engine,executablePath] of [['chromium',chromium,process.env.CHROMIUM_EXECUTABLE],['webkit',webkit,process.env.WEBKIT_EXECUTABLE]]) {
 ...
 for(const [width,height] of (process.env.CONTRAST_DIAGNOSTIC ? [[1440,900],[1024,768]] : [[1440,900],[1280,800],[1024,768],[768,1024],[430,932],[390,844],[320,568]])) {
  ...
  for(const state of [{time:0},{time:3},{time:8},{time:14},{time:28},{time:56},{time:8,pointer:true},{time:28,static:true},...(width>780?[{time:28,tether:'lower'},{time:28,tether:'upper'},...[120,150,280,400].map(x=>({time:28,tether:'center',x})),{time:28,tether:'edge'}]:[])]) {
   await page.evaluate(state=>{
    ...
    const tether=document.querySelector('.hero-knowledge__tethers'); tether.style.display=state.tether?'block':'none';
    ContrastRenderer.drawIntakeTethers(tether.getContext('2d'),layout,r.width,r.height,{x:state.tether==='edge'?layout.cx-550*layout.scale-20:(state.x??(state.tether==='lower'?120:150))-r.x,y:layout.cy+(state.tether==='lower'?280:state.tether==='upper'?-210:0),strength:state.tether?1:0},!!state.static);
   },state);
   ...
   if(width===1024&&state.time===8&&!state.pointer){await writeFile(`${out}/${engineName}-1024-t8-background.png`,Buffer.from(bg,'base64'));await page.screenshot({path:`${out}/${engineName}-1024-t8.png`});}
...
if (results.some(r=>r.measured.some(m=>!m.meetsSampledThreshold))) process.exitCode=1;
