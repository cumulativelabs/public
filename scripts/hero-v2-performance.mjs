...
const samples=[];
for(const width of [1440,390]) for(let trial=0;trial<3;trial++) for(const [name,url] of [['main',process.env.BASELINE_URL||'http://127.0.0.1:4180/'],['v2',process.env.HERO_URL||'http://127.0.0.1:4179/']]) {
...
await browser.close();
const copyUnchanged=samples.filter(s=>s.name==='main').every(s=>samples.find(t=>t.name==='v2'&&t.width===s.width&&t.trial===s.trial)?.copy===s.copy);
...
await writeFile(fileURLToPath(new URL('../docs/website/hero-visual-v2/evidence/performance-results.json',import.meta.url)),JSON.stringify(report,null,2)+'\n');
