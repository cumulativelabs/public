...
for(const [name,engine] of [['chromium',chromium],['webkit',webkit]]){
 ...
 for(const [width,height,dpr,label,x,yarg] of cases){
  ...
  rows.push({engine:name,version:browser.version(),origin:process.env.HERO_URL,width,height,dpr,label,pointer:{x,y},mask,errors,measured});await writeFile(out+'/results.json',JSON.stringify(rows,null,2));
