...
export async function sampleContrast(page,out,label){
   const records=await page.evaluate(()=>{
    ...
    return selectors.flatMap(selector=>Array.from(document.querySelectorAll(selector)).filter(e=>e.getBoundingClientRect().width&&getComputedStyle(e).visibility!=='hidden').map(e=>{
     ...
     const walker=document.createTreeWalker(e,NodeFilter.SHOW_TEXT),rects=[];while(walker.nextNode()){if(!walker.currentNode.textContent.trim())continue;const range=document.createRange();range.selectNodeContents(walker.currentNode);for(const b of range.getClientRects())rects.push({x:b.x,y:b.y,width:b.width,height:b.height});}
     return{id,selector,text:e.textContent.trim(),fontSize:s.fontSize,fontWeight:s.fontWeight,color:s.color,fill:s.webkitTextFillColor,gradient:s.backgroundClip==='text'?s.backgroundImage:null,rect:{x:r.x,y:r.y,width:r.width,height:r.height},rects};
    }));
   });
   ...
   const screenshotDimensions={width:png.readUInt32BE(16),height:png.readUInt32BE(20),cssWidth:cssSize.width,cssHeight:cssSize.height};
   ...
   const bg=(await page.screenshot({fullPage:false})).toString('base64');await style.evaluate(e=>e.remove());
   ...
   const fg=(await page.screenshot({fullPage:false})).toString('base64');await gradStyle.evaluate(e=>e.remove());
   ...
   await writeFile(out+'/'+label+'-foreground.png',Buffer.from(fg,'base64'));
