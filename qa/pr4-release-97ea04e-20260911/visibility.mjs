...
for(const [name,engine,executablePath] of [['chromium',chromium,process.env.CHROMIUM_EXECUTABLE],['webkit',webkit,process.env.WEBKIT_EXECUTABLE]]){
 ...
 const p=await b.newPage({viewport:{width,height},deviceScaleFactor:1,reducedMotion:'no-preference'});
 ...
 const cases=[['center150',150,cy],['center280',280,cy],['edge',edge-20,cy]];
 for(const [label,x,y] of cases){await p.reload({waitUntil:'networkidle'});await p.evaluate(()=>document.fonts.ready);await p.mouse.move(x,y);await p.waitForTimeout(1250);const state=await p.evaluate(()=>{window.__freeze=true;const r=document.querySelector('.hero-knowledge').getBoundingClientRect(),e=document.querySelector('.hero-knowledge');return{bounds:r.toJSON(),dataset:{...e.dataset},strokes:window.__strokes}});await p.waitForTimeout(80);const file=`${name}-${width}-${label}`;await p.screenshot({path:out+'/'+file+'.png'});await p.locator('.hero-knowledge__tethers').evaluate(e=>e.style.visibility='hidden');await p.screenshot({path:out+'/'+file+'-hidden.png'});rows.push({engine:name,width,height,label,pointer:{x,y},...state,file});await writeFile(out+'/states.json',JSON.stringify(rows,null,2));}
