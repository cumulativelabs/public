...
for(const label of process.argv.slice(2)){
 ...
 const a=await decode(shown),b=await decode(hidden);const rx=a.width/s.width,ry=a.height/s.height;const rgb=(im,x,y)=>{const i=(Math.round(y*ry)*im.width+Math.round(x*rx))*4;return Array.from(im.data.slice(i,i+3));};
 ...
 if(X<5||Y<5||X>=s.width-5||Y>=s.height-5||Math.hypot(X-x,Y-y)>1.5||Math.hypot(X-s.pointer.x,Y-s.pointer.y)<22)continue;
