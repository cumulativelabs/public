...
The scene has five offset archival layers around the unchanged official nested-C asset. Small amber evidence families connect at the perimeter. Dotted alternatives are challenged and disappear; supported paths and selected nodes remain. Violet branches subsequently extend from retained structure. This is deterministic conceptual storytelling, not a display of research measurements or operational activity.

The visual is deliberately bounded: fewer than 150 nodes and 150 precomputed links, no all-pairs network, no WebGL, video, new image assets, visualization package, or dependency addition. Thin strokes, translucent layer surfaces, and sparse highlights replace the wave system.
